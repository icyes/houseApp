<template>
	<view v-if="isLoad" class="login">
		<view class="content">
			...
			<wButton 
				:bgColor="canlogin?'linear-gradient(to right, rgba(0,0,0,0.7), rgba(0,0,0,0.6))':'#8799a3'"
				text="登 录"
				:rotate="isRotate" 
				@click.native="submit()"
			></wButton>
...
<!-- 	<view v-if="isLoad" class="flex-sub flex flex-direction bg-gradual-blue">
		<cu-custom bgColor="bg-gradual-blue"><block slot="content">登陆</block></cu-custom>
		<view class="cu-card case flex-sub flex-column-center align-center margin-top-xl">
			<view class="login radius flex-column-center shadow-warp bg-white">
				<view style="width: 90%;"  class="flex  p-xs margin-bottom-sm mb-sm align-center solid-bottom ani-show">
				...
				<view style="width: 90%;" class="padding flex flex-direction ani-show"><button class="cu-btn  lg" :class="canlogin?'bg-cyan':'bg-grey'" @click="submit">登陆</button></view>
			</view>
		</view>
	</view> -->
</template>
